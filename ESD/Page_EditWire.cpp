...
IMPLEMENT_DYNAMIC(CPage_EditWire, CPropertyPage)
CPage_EditWire::CPage_EditWire()
	: CPropertyPage(CPage_EditWire::IDD)
	, m_nComboWidth(0)
...
void CPage_EditWire::DoDataExchange(CDataExchange* pDX)
{
	CPropertyPage::DoDataExchange(pDX);

	DDX_Control(pDX, IDC_STATIC_EDIT_WIRE, m_Bitmap);
	DDX_CBIndex(pDX, IDC_COMBO_WIDTH, m_nComboWidth);
	DDX_Check(pDX, IDC_CHECK_GLOBAL, m_bCheckGlobal);
}


BEGIN_MESSAGE_MAP(CPage_EditWire, CPropertyPage)
	ON_STN_CLICKED(IDC_STATIC_COLOR, OnStnClickedStaticColor)
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()


// CPage_EditWire 消息处理程序

BOOL CPage_EditWire::OnInitDialog()
{
	CPropertyPage::OnInitDialog();

	m_Bitmap.SetBitmap(IDB_EDIT_WIRE);
...
void CPage_EditWire::OnStnClickedStaticColor()
{
...
HBRUSH CPage_EditWire::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
